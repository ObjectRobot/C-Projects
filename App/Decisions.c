...
void decisions3(void){
    // declare a varibale of type float
    // initalize it using constant defined in header file
    float num = NUM;

    // declare a variable of type bool and initalize it
    bool isNotBetween = !(num > 0.0 && num < 40000.0); // false

    // use bool variable in an if-else
    if (isNotBetween) {
        printf("Num (%.2f) is not between 0.0 and 40000.0\n", num); // not displayed
    } else {
        printf("Num (%.2f) is between 0.0 and 40000.0\n", num); // displayed
    ...
    /* this function will use ternary operator */
    ...
    /* this function will examine short circuit evalution
   ...
   void decisions6(void){
        ...
        // short circuit evalution will not take place
        if (x != 1 || y){
        ...
        // short circuit evalution will take place
        if (x == 1 || y){
            printf("If blocked executed.\n"); // displayed
        } else {
            printf("Else block executed.\n");
